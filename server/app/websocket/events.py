...
class TaleEvents:
    ...
    async def on_message(self, db: AsyncSession, user_id: int, content: str) -> None:
        ...
        if not user:
            await self._emit_error(user_id, "Пользователь не найден")
            return

        try:
            result = await self._tale_service.add_message(db, tale, content, user)
    ...
    async def on_complete_request(self, db: AsyncSession, user_id: int) -> None:
        ...
        try:
            ...
            await self._emit_event(user_id, "tale_completed", {"full_text": full_text})
        except Exception as e:
            await self._emit_error(user_id, str(e))
